...
export default function Edit({ isMobile, onClose, isOpen }) {
  const { data: detailPendidikanTerakhir } = HookSwr({
    path: isOpen ? `/pendidikan-terakhir/${isOpen}` : '',
  })
  ...
  const onFinish = (values) => {
    setLoading(true)
    updateApi({
      endpoint: `/pendidikan-terakhir/update/${isOpen}`,
      payload: values,
    })
      .then((res) => {
        form.resetFields()
  ...
  useEffect(() => {
    if (isOpen) {
      form.setFieldsValue({
        title: detailPendidikanTerakhir?.data?.title,
      })
    }
  }, [isOpen, form, detailPendidikanTerakhir])
