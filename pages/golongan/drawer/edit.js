...
export default function Edit({ onClose, isOpen }) {
  const { data: detailGolongan } = HookSwr({
    path: isOpen ? `/golongan/${isOpen}` : '',
  })
  ...
  const onFinish = (values) => {
    setLoading(true)
    updateApi({
      endpoint: `/golongan/update/${isOpen}`,
      payload: values,
    })
      .then((res) => {
        form.resetFields()
  ...
  useEffect(() => {
    if (isOpen) {
      form.setFieldsValue({
        title: detailGolongan?.data?.title,
      })
    }
  }, [isOpen, form, detailGolongan])
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Input size="large" placeholder="Title ..." maxLength={7} />
        </Form.Item>
        <Form.Item hidden>
